...
public:
    ...
    NumarComplex operator / (const NumarComplex &A)
    {
        ...
        double slash=A.parte_reala*A.parte_reala+A.parte_imaginara*A.parte_imaginara;
...
private:
    ...
    int *L=NULL;
    int *C=NULL;
    NumarComplex *Matrix=NULL;
    NumarComplex calcul_determinant1x1(Matrice M)
    {
        if(M.crt==1) return M.Matrix[0];
        NumarComplex Anulare(0,0);
        return Anulare;
    }
    NumarComplex calcul_det_mat(Matrice M)
    {
        NumarComplex MO[3][3];
                    ...
                    if(M.L[x]==i&&M.C[x]==j)
                    {
                        ...
                        for(int k=0;k<M.n;++k)
                            for(int l=0;l<M.n;++l)
                            {
                                ...
                                newM.Matrix=(NumarComplex*) realloc(newM.Matrix,newM.crt*sizeof(NumarComplex));
...
public:
        ...
        for(int i=0;i<A.n;++i)
            for(int j=0;j<A.m;++j)
            {
                ...
                if(aux!=Anulare)
                {
                    ...
                    rez.Matrix=(NumarComplex*) realloc(rez.Matrix,rez.crt*sizeof(NumarComplex));
                    rez.L[rez.crt-1]=i;
                    rez.C[rez.crt-1]=j;
    ...
    Matrice operator = (const Matrice &A)
    {
        ...
        Matrix=(NumarComplex*)realloc(Matrix,crt*sizeof(NumarComplex));
        for(int i=0;i<crt;++i)
        {
            L[i]=A.L[i];
            C[i]=A.C[i];
    ...
    NumarComplex determinant_matrice()
    {
        NumarComplex Anulare(0,0);
        if(n!=m) throw 10;
        /*
    ...
    Matrice inversa_matricii()
    {
        ...
        ///*/
        NumarComplex det=determinant_matrice();
        ...
        for(int i=0;i<A.n;++i)
            for(int j=0;j<A.m;++j)
            {
                ...
                if(Anulare!=aux)
                {
                    ...
                    A.Matrix=(NumarComplex*) realloc(A.Matrix,A.crt*sizeof(NumarComplex));
                    A.L[A.crt-1]=i;
                    A.C[A.crt-1]=j;
